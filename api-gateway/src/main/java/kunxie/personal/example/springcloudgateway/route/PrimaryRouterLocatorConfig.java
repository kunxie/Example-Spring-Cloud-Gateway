...
/**
 * The RouteLocator creation for PRIMARY route strategy.
 */
@Slf4j
@Configuration
public class PrimaryRouterLocatorConfig {

  /**
   * Configures route locator for PRIMARY based routing strategy using the provided configurations.
   * <p>
   * Note: always route to PRIMARY url
   *
   ...
   * @return a configured RouteLocator based on the PRIMARY strategy
   */
  @Bean
  public RouteLocator primaryRouterLocator(RouteLocatorBuilder routeLocatorBuilder, RouteApiConfigs apiConfigs) {
    ...
    apiConfigs.getApiConfigs().stream()
        .filter(apiConfig -> RouteStrategy.PRIMARY.equals(apiConfig.getRouteStrategy()))
